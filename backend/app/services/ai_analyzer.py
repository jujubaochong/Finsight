...
{{
  ...
  "phase": "判断主力当前所处阶段，从以下四选一：建仓吸筹/启动拉升/离场派发/观望不明",
  ...
  "phase_reason": "为什么判断主力处于该阶段，结合主力净额、量价、均线说明（2-3句）",
...
== 主力阶段判断口径 ==
- 建仓吸筹：主力持续净流入但股价温和未拉升、换手不高
- 启动拉升：主力净流入 + 放量上涨 + 均线多头 + 可能MACD金叉
- 离场派发：主力持续净流出，常伴股价滞涨/下跌、均线走弱
- 观望不明：多空交织、信号矛盾时
（输入里的"主力阶段规则研判"是系统按规则给的参考，你可结合数据认同或修正它）
...
def short_term_analysis(code: str, name: str, industry: str, snapshot: dict) -> dict:
    ...
    context = {
        ...
        "主力阶段规则研判": snapshot.get("main_phase", {}),
    ...
    for attempt in range(settings.ai_max_retries):
        try:
            ...
            result.setdefault("phase", "观望不明")
            result.setdefault("technical", "")
            result.setdefault("capital", "")
            result.setdefault("phase_reason", "")
